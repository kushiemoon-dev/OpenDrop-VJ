...
/// `rate` within the beat cycle (`clock_phase01`): same "multiplier of
/// beat rate" convention as `lfo::LfoSlot::rate`, so `rate == 2` flashes
/// twice per beat and `rate == 0.25` once every 4 beats. `now_sec` is
/// unused on this path: `clock_phase01` is the synced source of truth for
/// where the beat is (MIDI clock / Ableton Link / the audio beat detector
/// all drive it, see `Show::clock`), independent of this process's own
/// uptime.
///
/// **No tempo** (`clock_bpm <= 0`, e.g. nothing detected/set yet: see
/// `Show::current_bpm`): `clock_phase01` is frozen (`Clock::step` never
/// advances phase at bpm 0), so there is no beat to sync a flash to.
/// Falls back to a free-running pulse at `rate` Hz, driven by `now_sec`
/// instead, so toggling Strobe on still does something visible before a
/// tempo locks.
pub fn strobe_flash_intensity(state: &StrobeState, clock_phase01: f64, clock_bpm: f64, now_sec: f64) -> f32 {
    ...
    let time_since_edge_sec = if clock_bpm > 0.0 {
        (clock_phase01 * (60.0 / clock_bpm)).rem_euclid(sub_period_sec)
    } else {
...
mod tests {
    ...
    mod tempo_synced {
        ...
        fn reaches_exactly_0_at_the_flash_length_boundary() {
            let state = enabled(1.0, 0.8);
            // time_since_edge = 0.08s -> phase01 = 0.08 / 0.5 = 0.16.
        ...
        fn rate_2_adds_a_second_trigger_mid_beat() {
            ...
            // sub-period = 60/(120*2) = 0.25s -> edges at phase01 0.0 and 0.5.
            assert_eq!(strobe_flash_intensity(&state, 0.5, 120.0, 0.0), 0.8);
            // Halfway between edges (phase01 0.25) is well past the 0.08s
        ...
        fn flash_length_is_capped_to_half_the_sub_period_at_a_fast_rate() {
            ...
            // time_since_edge = 0.01s -> phase01 = 0.01 / (60/300) = 0.05.
            let v = strobe_flash_intensity(&state, 0.05, 300.0, 0.0);
            assert!((v - 0.6).abs() < 1e-6, "{v}"); // decay = 1 - 0.01/0.025
            // time_since_edge = 0.025s (the capped boundary) -> phase01 = 0.125.
