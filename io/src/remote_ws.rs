...
async fn serve_until_stopped(
    ...
    loop {
        ...
        let app = build_router(token, events_tx.clone());
...
fn build_router(token: String, events_tx: Sender<(CommandId, f64)>) -> Router {
    let assets = assets_dir();
    let index_html = assets.join("index.html");
    Router::new()
        .route("/", get(ws_handler))
        .with_state(WsState { token, events_tx })
...
async fn handle_socket(mut socket: WebSocket, state: WsState) {
    while let Some(Ok(msg)) = socket.recv().await {
        ...
        // Binary/Ping/Pong/Close frames: nothing to do: mirrors the JS
        // reference, which only ever handles `ws.on('message', ...)`
        // (text/JSON) and lets the `ws` library's own automatic ping/pong
        // and close handling take care of the rest.
    }
}
