...
async function main() {
  ...
  if (megapackAbsent) {
    const existingFiles = (await readdir(OUT_DIR)).filter(
    ...
    if (preserved > 0) {
      ...
      console.log(
        `   Pour des noms complets, relancer avec : MEGAPACK_DIR=/chemin/vers/converted node scripts/build-presets.mjs`
      )
    }
  }
