...
async function fetchPexels(term, count, apiKey) {
	...
	return (data.videos ?? []).slice(0, count).map((v) => {
...
async function main() {
	...
	for (const query of sources.queries) {
		...
		if (!DRY_RUN) {
			try {
				if (provider === 'pexels') {
					if (!PEXELS_KEY) { console.warn('  ⚠️  PEXELS_API_KEY non défini — skipped'); continue; }
					videoUrls = await fetchPexels(term, count, PEXELS_KEY);
				} else if (provider === 'pixabay') {
					if (!PIXABAY_KEY) { console.warn('  ⚠️  PIXABAY_API_KEY non défini — skipped'); continue; }
		...
		for (let i = 0; i < videoUrls.length; i++) {
			...
			const name = `${term.split(' ').map((w) => w[0].toUpperCase() + w.slice(1)).join(' ')} ${String(i + 1).padStart(2, '0')}`;
