...
declare global {
  ...
  interface Window {
    electronAPI?: {
      ...
      ndiFind: () => Promise<{
        ok: boolean
        sources: Array<{ name: string; urlAddress: string }>
        error?: string
      }>
      ndiReceiveStart: (
        name: string,
      ...
      startLoopback: (
        deviceId: number
      ) => Promise<{ ok: boolean; sampleRate?: number; channels?: number; error?: string }>
      stopLoopback: () => Promise<{ ok: boolean }>
      onLoopbackData: (
        cb: (data: { sampleRate: number; channels: number; pcm: Uint8Array }) => void
      ) => () => void
      sendAudioFrame: (data: { sampleRate: number; channels: number; pcm: Int16Array }) => void
      onAudioFrame: (
        cb: (data: { sampleRate: number; channels: number; pcm: Int16Array }) => void
      ) => () => void
      ...
      onLinkState: (
        cb: (state: { tempo: number; beat: number; phase: number; peers: number }) => void
      ) => () => void
