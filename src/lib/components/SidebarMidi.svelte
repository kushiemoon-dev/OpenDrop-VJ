<script lang="ts">
	...
	import { formatTrigger, type MidiTriggerKey } from '$lib/engine/midi.js';
	...
	interface Props {
		...
		midiClockBpm: number;
		learningAction: CommandId | null;
		midiMappings: Partial<Record<CommandId, MidiTriggerKey>>;
		registry: CommandRegistry;
		onToggleMidi: () => void;
		onStartLearn: (id: CommandId) => void;
		onClearMapping: (id: CommandId) => void;
	}

	let {
		midiSupported, midiConnected, midiDeviceNames, midiClockBpm, learningAction, midiMappings, registry,
		onToggleMidi, onStartLearn, onClearMapping,
	}: Props = $props();
</script>
			...
			{#each registry.all() as cmd}
				...
				<div class="midi-row">
					...
					<button class="btn-sm pl-btn" class:active={learningAction === cmd.id}
						onclick={() => onStartLearn(cmd.id)}>
						{learningAction === cmd.id ? '…' : 'Learn'}
					</button>
					{#if mapped}
						<button class="pl-remove" onclick={() => onClearMapping(cmd.id)}>×</button>
					{/if}
				</div>
			{/each}
		</div>
