  ...
  try {
    ...
    if (isElectron && platform === 'win32') {
      ...
      if (monitors.length === 1 && monitor) {
        await audio.connectDevice(monitor.deviceId)
        audioSourceState.currentDeviceId = monitor.deviceId
        audioSourceState.sourceLabel = monitor.label || 'system audio'
        sync?.sendSource(monitor.deviceId)
