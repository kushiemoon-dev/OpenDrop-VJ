...
export function onBeat(
  sync: MainSync | null,
  clock: Clock,
  applyMidiAction: (id: CommandId, value: number) => void
): void {
  ...
  if (
    beatSyncState.beatSyncA &&
    !beatSyncState.lockA &&
    shouldTriggerOnBeat(clock.beatCount, beatSyncState.beatTriggerA)
  ) {
    if (playlistState.aItems.length > 0) playlistNext('A')
    else applyMidiAction('preset-next-a', 127)
  }
  if (
    beatSyncState.beatSyncB &&
    !beatSyncState.lockB &&
    shouldTriggerOnBeat(clock.beatCount, beatSyncState.beatTriggerB)
  ) {
    if (playlistState.bItems.length > 0) playlistNext('B')
    else applyMidiAction('preset-next-b', 127)
  }
  if (
    overlayState.queueEnabled &&
    shouldTriggerOnBeat(clock.beatCount, overlayState.queueTrigger)
  ) {
...
export function toggleBeatSync(deck: 'A' | 'B'): void {
  if (deck === 'A') {
    ...
    setPlaylistBeatSyncInterval(
      'A',
      beatSyncState.beatSyncA ? Infinity : playlistState.intervalSec * 1000
    )
  } else {
    beatSyncState.beatSyncB = !beatSyncState.beatSyncB
    setPlaylistBeatSyncInterval(
      'B',
      beatSyncState.beatSyncB ? Infinity : playlistState.intervalSec * 1000
    )
  }
}
