...
import {
  ...
  initCloudPresets,
  refreshCloudPresets,
  onCloudPresetFilePick,
  copyCloudToken,
  linkCloudDevice,
  renameCloudPresetEntry,
  deleteCloudPresetEntry,
...
describe('cloud-presets-store', () => {
  ...
  describe('renameCloudPresetEntry', () => {
    it('renames then refreshes', async () => {
      ...
      expect(cloudPresetsApi.renameCloudPreset).toHaveBeenCalledWith(
        'token-abc',
        'id-1',
        'Nouveau nom'
      )
  ...
  describe('onCloudPresetFilePick', () => {
    ...
    it('uploads the parsed file (name without .json extension) and refreshes on success', async () => {
      ...
      expect(cloudPresetsApi.uploadPreset).toHaveBeenCalledWith('token-abc', 'my-preset', {
        frame_eqs_str: 'a=1;',
      })
      expect(cloudPresetsApi.getCloudPresetIndex).toHaveBeenCalled()
