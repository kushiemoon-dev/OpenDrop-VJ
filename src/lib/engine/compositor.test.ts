...
import {
  blendStateFor,
  blendModeFromValue01,
  blendModeToValue01,
  migrateBlendModeString,
  GLBlend,
  withSlotComposite,
  type SlotComposites,
  isVideoLayerActive,
  shouldForceNormalForLowestSlot,
} from './compositor.js'
...
describe('blendStateFor', () => {
  it('normal → classic over (SRC_ALPHA coverage constant)', () => {
    expect(blendStateFor('normal')).toEqual({
      ...
      srcA: GLBlend.ONE,
      dstA: GLBlend.ONE_MINUS_SRC_ALPHA,
    })
  })
  ...
  it('screen → ONE/ONE_MINUS_SRC_COLOR in RGB', () => {
    expect(blendStateFor('screen')).toEqual({
      srcRGB: GLBlend.ONE,
      dstRGB: GLBlend.ONE_MINUS_SRC_COLOR,
      srcA: GLBlend.ONE,
      dstA: GLBlend.ONE_MINUS_SRC_ALPHA,
    })
  })
...
describe('blendModeFromValue01', () => {
  it('0 → normal', () => {
    expect(blendModeFromValue01(0)).toBe('normal')
  })
  it('0.24 → normal', () => {
    expect(blendModeFromValue01(0.24)).toBe('normal')
  })
  it('0.25 → additive', () => {
    expect(blendModeFromValue01(0.25)).toBe('additive')
  })
  it('0.5 → screen', () => {
    expect(blendModeFromValue01(0.5)).toBe('screen')
  })
  it('0.75 → multiply', () => {
    expect(blendModeFromValue01(0.75)).toBe('multiply')
  })
  it('1 → multiply (clamped)', () => {
    expect(blendModeFromValue01(1)).toBe('multiply')
  })
  it('negative out-of-range value → normal (clamped)', () => {
    expect(blendModeFromValue01(-0.5)).toBe('normal')
  })
})
...
describe('migrateBlendModeString', () => {
  it('screen → screen', () => {
    expect(migrateBlendModeString('screen')).toBe('screen')
  })
  it('multiply → multiply', () => {
    expect(migrateBlendModeString('multiply')).toBe('multiply')
  })
  it('plus-lighter → additive', () => {
    expect(migrateBlendModeString('plus-lighter')).toBe('additive')
  })
  it('overlay (unsupported) → normal', () => {
    expect(migrateBlendModeString('overlay')).toBe('normal')
  })
  it('lighten (unsupported) → normal', () => {
    expect(migrateBlendModeString('lighten')).toBe('normal')
  })
  it('any unknown value → normal', () => {
    expect(migrateBlendModeString('garbage')).toBe('normal')
  })
})
