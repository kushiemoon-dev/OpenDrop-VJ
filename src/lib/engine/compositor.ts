...
export function withSlotComposite(
  composites: SlotComposites,
  slot: number,
  patch: Partial<SlotComposite>
): SlotComposites {
 ...
 */
export function isVideoLayerActive(
  hasSource: boolean,
  opacity: number,
  readyState: number
): boolean {
...
export class Compositor {
  ...
  private drawFrame(): void {
    ...
    for (let slot = 0; slot < SLOT_COUNT; slot++) {
      ...
      if (!layer || !layer.source || layer.opacity <= 0.001) continue

      // Force normal on the lowest active layer — multiply against a
      // transparent framebuffer would otherwise read as black.
      const mode: BlendMode = shouldForceNormalForLowestSlot(slot, lowestActive)
        ? 'normal'
        : layer.config.blend
      const bs = blendStateFor(mode)
      gl.blendFuncSeparate(
        glFactor(gl, bs.srcRGB),
        glFactor(gl, bs.dstRGB),
        glFactor(gl, bs.srcA),
        glFactor(gl, bs.dstA)
      )
    ...
    const videoActive = isVideoLayerActive(
      this.videoSource !== null,
      this.videoOpacity,
      this.videoSource?.readyState ?? 0
    )
    if (videoActive) {
      const vbs = blendStateFor('normal')
      gl.blendFuncSeparate(
        glFactor(gl, vbs.srcRGB),
        glFactor(gl, vbs.dstRGB),
        glFactor(gl, vbs.srcA),
        glFactor(gl, vbs.dstA)
      )
      gl.blendEquation(gl.FUNC_ADD)
