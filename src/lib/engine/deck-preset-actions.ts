...
export async function loadImportedMilkPreset(
  ...
  if (bus === 'A') {
    const p = primaryPreset('A')
    sync?.sendPreset('A', p, deckState.transitionTime, p === name ? data : undefined)
  } else if (bus === 'B') {
    const p = primaryPreset('B')
    sync?.sendPreset('B', p, deckState.transitionTime, p === name ? data : undefined)
  }
}
