...
/** engine.tick() always returns one entry per slot (LfoEngine has a fixed slot count), so [0] is safe. */
function tickFirst(engine: LfoEngine, phase: number) {
  return engine.tick(phase)[0]!
}
...
describe('LfoEngine — sine', () => {
  it('sine equals 0.5 at phase=0', () => {
    ...
    const { value01 } = tickFirst(engine, 0)
  ...
  it('sine equals 1 at phase=0.25 (peak)', () => {
    ...
    const { value01 } = tickFirst(engine, 0.25) // sin(π/2)=1 → raw=1 → val=0.5+(0.5)*1=1
  ...
  it('sine equals 0 at phase=0.75 (trough)', () => {
    ...
    const { value01 } = tickFirst(engine, 0.75) // sin(3π/2)=-1 → raw=0 → val=0.5-0.5=0
...
describe('LfoEngine — saw', () => {
  it('saw with center=0.5, amount=1 → range 0..1', () => {
    ...
    expect(tickFirst(engine, 0.6).value01).toBeCloseTo(0.6)
    expect(tickFirst(engine, 0).value01).toBeCloseTo(0) // raw=0 → 0.5-0.5=0
    expect(tickFirst(engine, 1).value01).toBeCloseTo(0) // raw=1%1=0 → 0
  })
})
...
describe('LfoEngine — square', () => {
  it('square equals 1 in the first half', () => {
    ...
    const { value01 } = tickFirst(engine, 0.25) // p=0.25 < 0.5 → raw=1 → val=0.5+0.5=1
  ...
  it('square equals 0 in the second half', () => {
    ...
    const { value01 } = tickFirst(engine, 0.75) // raw=0 → val=0.5-0.5=0
...
describe('LfoEngine — rate', () => {
  it('rate=2 doubles the frequency (full cycle at phase=0.5)', () => {
    ...
    expect(tickFirst(engine, 0.5).value01).toBeCloseTo(0)
  })
})
...
describe('LfoEngine — amount + center', () => {
  it('amount=0.5 limits the deviation to ±0.25 of center', () => {
    ...
    const peak = tickFirst(engine, 0.25).value01 // raw=1 → 0.5+(1-0.5)*0.5=0.75
    const trough = tickFirst(engine, 0.75).value01 // raw=0 → 0.5+(0-0.5)*0.5=0.25
  ...
  it('values clamped to 0..1', () => {
    ...
    const values = [0, 0.25, 0.5, 0.75].map((p) => tickFirst(engine, p).value01)
...
describe('LfoEngine — disabled slot', () => {
  it('disabled slot returns center as value, target null', () => {
    ...
    const result = tickFirst(engine, 0.5)
...
describe('LfoEngine — randomizeSH', () => {
  it('s&h returns the memorized value', () => {
    ...
    engine.randomizeSH()
    const v1 = tickFirst(engine, 0).value01
    const v2 = tickFirst(engine, 0.5).value01
