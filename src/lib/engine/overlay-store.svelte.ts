...
export async function addOverlayAtPosition(
  name: string,
  dataUrl: string,
  x: number,
  y: number
...
export async function removeOverlay(id: string): Promise<void> {
  ...
  overlayState.queueIndex = clampQueueIndex(
    overlayState.queueIndex,
    pickQueuedOverlays(overlayState.overlays).length
  )
}
