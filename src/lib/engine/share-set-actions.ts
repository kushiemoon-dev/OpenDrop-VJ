...
export async function selectPresetForDeck(
  deck: 'A' | 'B',
  name: string,
  manager: DeckManager,
  sync: MainSync | null
...
export async function copyShareLink(): Promise<void> {
  ...
  setTimeout(() => {
    shareSetState.copyLabel = 'Copy link'
  }, 1500)
}

export async function applyPendingSharedSet(
  manager: DeckManager,
  sync: MainSync | null
): Promise<void> {
  ...
  deckState.crossfader = s.crossfader
  deckState.transitionTime = s.transitionTime
  colorState.a = s.colorParamsA
  colorState.b = s.colorParamsB
  ...
  for (let slot = 0; slot < 4; slot++)
    Object.assign(getGlobalTimeParams()[slot]!, timeParamsState.params[slot]!)
  qvarState.params = s.qVarParams as typeof qvarState.params
  for (let slot = 0; slot < 4; slot++)
    Object.assign(getGlobalQVarParams()[slot]!, {
      enabled: [...qvarState.params[slot]!.enabled],
      value: [...qvarState.params[slot]!.value],
    })
  snapshotsState.snapshots = s.snapshots
  snapshotsState.recallDuration = s.snapshotRecallDuration
  timelineState.keyframes = s.timelineKeyframes
  overlayState.overlays = s.overlays
  beatSyncState.beatTriggerA = s.beatTriggerA
  beatSyncState.beatTriggerB = s.beatTriggerB
  beatSyncState.beatSyncA = s.beatSyncA
  beatSyncState.beatSyncB = s.beatSyncB
  overlayState.queueEnabled = s.overlayQueueEnabled
  overlayState.queueTrigger = s.overlayQueueTrigger
